...
public class ForStIncrementalRestoreOperation<K> implements ForStRestoreOperation {
    ...
    public ForStRestoreResult restore() throws Exception {
        ...
        StateHandleTransferSpec mainSpec =
                new StateHandleTransferSpec(
                        mainHandle, new Path(forstBasePath, ForStResourceContainer.DB_DIR_STRING));
    ...
    private void transferAllStateHandles(
        ...
        try (ForStStateDataTransfer transfer =
            ...
            List<StateHandleTransferSpec> specs = new ArrayList<>(otherSpecs.size() + 1);
            specs.add(mainSpecs);
    ...
    private void restoreFromMainTransferredHandle(StateHandleTransferSpec mainHandel)
            throws Exception {
        IncrementalRemoteKeyedStateHandle handle = mainHandel.getStateHandle();
